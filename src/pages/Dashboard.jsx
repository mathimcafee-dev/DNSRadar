...
function AutoFixButton({ domainId, issueType, fixValue, domainName }) {
  ...
  if (state === 'no-cred') return (
    <a href="#" onClick={e=>{e.preventDefault();setState('idle')}} style={{fontSize:10,color:'#ffc45e',textDecoration:'none',whiteSpace:'nowrap'}}>
  ...
  return (
    <div style={{flexShrink:0}}>
      {showConfirm && (
        <div style={{position:'absolute',right:16,zIndex:50,background:'#121929',border:'1px solid rgba(255,255,255,0.12)',borderRadius:10,padding:'12px 14px',width:260,boxShadow:'0 8px 32px rgba(0,0,0,0.4)'}}>
          <div style={{fontSize:12,fontWeight:600,color:'#fff',marginBottom:5}}>Push to {cred?.provider}?</div>
          <div style={{fontSize:10,fontFamily:'monospace',color:'rgba(255,255,255,0.45)',marginBottom:2}}>Type: {mapping.type} · Name: {typeof mapping.name==='function'?mapping.name(domainName):mapping.name}</div>
          ...
          <div style={{display:'flex',gap:6}}>
            <button onClick={execute} style={{flex:1,padding:'6px',background:'#22d9a0',color:'#fff',border:'none',borderRadius:6,cursor:'pointer',fontSize:11,fontWeight:600}}>Confirm push</button>
            <button onClick={()=>setShowConfirm(false)} style={{padding:'6px 10px',background:'rgba(255,255,255,0.06)',color:'rgba(255,255,255,0.6)',border:'none',borderRadius:6,cursor:'pointer',fontSize:11}}>Cancel</button>
          </div>
        </div>
      )}
      <button onClick={state==='idle'?loadCred:undefined} disabled={state==='loading'||state==='success'}
        style={{padding:'4px 10px',background:state==='success'?'rgba(16,185,129,0.15)':state==='error'?'rgba(239,68,68,0.15)':'rgba(245,158,11,0.12)',border:`1px solid ${state==='success'?'rgba(16,185,129,0.3)':state==='error'?'rgba(239,68,68,0.3)':'rgba(245,158,11,0.3)'}`,borderRadius:6,color:state==='success'?'#22d9a0':state==='error'?'#ff6b6b':'#ffc45e',fontSize:10,fontWeight:600,cursor:state==='idle'?'pointer':'default',whiteSpace:'nowrap',display:'flex',alignItems:'center',gap:4}}>
        {state==='loading'&&<div style={{width:10,height:10,border:'2px solid rgba(245,158,11,0.3)',borderTopColor:'#ffc45e',borderRadius:'50%',animation:'dsh-spin 0.7s linear infinite'}}/>}
        {state==='idle'&&'⚡ Fix auto'}
...
function Gauge({ score, size = 160 }) {
  ...
  const c=score>=70?'#22d9a0':score>=50?'#ffc45e':'#ff6b6b'
...
function DeleteModal({ domain, onConfirm, onCancel, loading }) {
  return (
    <div onClick={e=>e.target===e.currentTarget&&onCancel()}
      ...
      <div style={{background:'#121929',border:'1px solid rgba(255,255,255,0.1)',borderRadius:16,maxWidth:400,width:'100%',margin:'0 16px',padding:24}}>
        ...
        <div style={{fontSize:13,color:'rgba(255,255,255,0.45)',marginBottom:20}}>
          Permanently delete <span style={{color:'#ff6b6b',fontFamily:'monospace'}}>{domain?.domain_name}</span> and all scan history.
        </div>
        <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
          <button onClick={onCancel} style={{padding:'8px 16px',background:'rgba(255,255,255,0.06)',color:'rgba(255,255,255,0.7)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:8,cursor:'pointer',fontSize:13}}>Cancel</button>
          <button onClick={onConfirm} disabled={loading} style={{padding:'8px 16px',background:'#ff6b6b',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',fontSize:13,fontWeight:500}}>
...
function SBadge({ status }) {
  ...
  return (
    <span style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:`rgba(${pass?'16,185,129':fail?'239,68,68':'245,158,11'},0.15)`,color:pass?'#22d9a0':fail?'#ff6b6b':'#ffc45e',fontWeight:500,whiteSpace:'nowrap'}}>
      {status||'–'}
    </span>
  )
}
...
export default function Dashboard({ user, setPage, setScanDomain, setScanType, onDomainSelect }) {
  ...
  const cardHd={padding:'11px 16px',borderBottom:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',justifyContent:'space-between',background:D.surface2}

  const cats=scan?[
    {label:'DNS records',icon:Globe,color:'#60a5fa',bg:'#1e3a5f',score:scan.score_dns,max:25},
    {label:'Email auth',icon:Mail,color:'#ff6b6b',bg:'#3d1515',score:scan.score_email,max:30},
    {label:'SSL / TLS',icon:Lock,color:'#22d9a0',bg:'#0d2d23',score:scan.score_ssl,max:20},
    {label:'Propagation',icon:Globe,color:'#ffc45e',bg:'#3d2d0a',score:scan.score_propagation,max:10},
    {label:'Security',icon:Shield,color:'#c084fc',bg:'#2d1f4d',score:scan.score_security,max:10},
    {label:'Blacklists',icon:Ban,color:'#ff6b6b',bg:'#3d1515',score:scan.score_blacklist,max:5},
  ]:[]
  ...
  return (
    <div style={{display:'flex',height:'calc(100vh - 56px)',background:D.bg,fontFamily:"'DM Sans','Inter',system-ui,sans-serif"}}>
      ...
      {/* ── SIDEBAR ──────────────────────────────────────────── */}
      <div className="no-print" style={{width:220,flexShrink:0,background:D.surface,borderRight:'1px solid rgba(255,255,255,0.06)',display:'flex',flexDirection:'column'}}>
        <div style={{padding:12,borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
          <button onClick={()=>setShowAdd(true)}
            style={{width:'100%',padding:'8px 12px',background:'rgba(16,185,129,0.15)',border:'1px solid rgba(16,185,129,0.3)',borderRadius:8,color:'#22d9a0',fontSize:13,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',gap:6,justifyContent:'center'}}>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'6px 0'}}>
          ...
          :domains.map(d=>{
            ...
            const sc=score>=70?'#22d9a0':score>=50?'#ffc45e':'#ff6b6b'
            ...
            return (
              <div key={d.id} className="dsh-row" onClick={()=>{setSelected(d); onDomainSelect?.(d); setActiveTab('overview')}}
                style={{display:'flex',alignItems:'center',gap:8,padding:'8px 12px',cursor:'pointer',background:isActive?'rgba(16,185,129,0.07)':'transparent',borderLeft:`2px solid ${isActive?'#22d9a0':'transparent'}`}}>
                <div style={{width:7,height:7,borderRadius:'50%',background:d.paused?'rgba(255,255,255,0.22)':!d.verified?'#ffc45e':sc,flexShrink:0}}/>
                ...
                <div onClick={e=>{e.stopPropagation();setDeleteTarget(d)}} style={{color:'rgba(255,255,255,0.22)',cursor:'pointer',padding:2}}><Trash2 size={11}/></div>
              </div>
            )
          })}
        </div>
        {domains.length>0&&(()=>{
          ...
          const c=avg>=70?'#22d9a0':avg>=50?'#ffc45e':'#ff6b6b'
          return (
      ...
      <div style={{flex:1,overflowY:'auto',background:D.bg}}>
        {!selected?(
          <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100%',flexDirection:'column',gap:16}}>
            ...
            <div style={{fontSize:18,fontWeight:500,color:'rgba(255,255,255,0.22)'}}>Add a domain to get started</div>
            <button onClick={()=>setShowAdd(true)} style={{padding:'10px 24px',background:'#22d9a0',color:'#fff',border:'none',borderRadius:9,fontSize:14,fontWeight:500,cursor:'pointer',display:'flex',alignItems:'center',gap:7}}><Plus size={15}/> Add your first domain</button>
          </div>
        ):(
          <div>
            {/* ── DOMAIN HEADER ── */}
            <div className="no-print" style={{padding:'14px 20px',borderBottom:'1px solid rgba(255,255,255,0.06)',background:D.surface}}>
              <div style={{display:'flex',alignItems:'flex-start',gap:16,flexWrap:'wrap'}}>
                <div style={{flex:1}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4,flexWrap:'wrap'}}>
                    <h2 style={{fontSize:17,fontWeight:700,color:D.text,margin:0}}>{selected.domain_name}</h2>
                    {selected.verified&&<span style={{fontSize:10,padding:'2px 8px',borderRadius:10,background:'rgba(16,185,129,0.15)',color:'#22d9a0',border:'1px solid rgba(16,185,129,0.25)',fontWeight:500}}>Verified</span>}
                    {selected.paused&&<span style={{fontSize:10,padding:'2px 8px',borderRadius:10,background:'rgba(255,255,255,0.06)',color:D.muted,fontWeight:500}}>Paused</span>}
                    {critical.length>0&&<span style={{fontSize:10,padding:'2px 8px',borderRadius:10,background:'rgba(239,68,68,0.15)',color:'#ff6b6b',border:'1px solid rgba(239,68,68,0.2)',fontWeight:500}}>{critical.length} critical</span>}
                    {warns.length>0&&<span style={{fontSize:10,padding:'2px 8px',borderRadius:10,background:'rgba(245,158,11,0.15)',color:'#ffc45e',fontWeight:500}}>{warns.length} warnings</span>}
                  </div>
                  ...
                  <div style={{display:'flex',gap:0,borderBottom:'1px solid rgba(255,255,255,0.06)',marginBottom:-14}}>
                    {tabs.map(t=>(
                      <button key={t} className="dsh-tab" onClick={()=>setActiveTab(t)}
                        style={{padding:'8px 14px',background:'transparent',border:'none',borderBottom:`2px solid ${activeTab===t?'#22d9a0':'transparent'}`,cursor:'pointer',fontSize:12,fontWeight:activeTab===t?600:400,color:activeTab===t?'#22d9a0':'rgba(255,255,255,0.45)',textTransform:'capitalize',transition:'all 0.15s',marginBottom:-1}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:6,alignSelf:'flex-start'}}>
                  <button onClick={()=>triggerScan(selected)} disabled={scanning[selected.id]}
                    style={{padding:'6px 14px',background:'#22d9a0',color:'#fff',border:'none',borderRadius:8,cursor:'pointer',fontSize:12,fontWeight:600,display:'flex',alignItems:'center',gap:5}}>
                ...
                <>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:10}}>
                    {[
                      ...
                      {label:'Critical issues',val:critical.length,color:critical.length>0?'#ff6b6b':'#22d9a0',sub:critical.length>0?'Fix immediately':'All clear',pct:Math.min(critical.length*25,100)},
                      {label:'Blacklisted',val:`${scan.blacklists?.listed_count||0}/${scan.blacklists?.results?.length||0}`,color:(scan.blacklists?.listed_count||0)>0?'#ff6b6b':'#22d9a0',sub:'blacklists',pct:(scan.blacklists?.listed_count||0)>0?60:100},
                      {label:'DNS records',val:scan.dns_records?.length||0,color:'#60a5fa',sub:'records found',pct:100},
                    ].map(k=>(
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
                    <div className="print-card" style={{...card,padding:'14px 16px'}}>
                      <div style={{fontSize:12,fontWeight:600,color:D.text,marginBottom:12,display:'flex',alignItems:'center',gap:6}}>
                        <span style={{width:3,height:14,background:'#22d9a0',borderRadius:2,display:'inline-block'}}/>
                        Score history
                      </div>
                  ...
                  <div className="print-card" style={{...card}}>
                    <div style={{...cardHd}}>
                      ...
                      <span style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:'rgba(239,68,68,0.15)',color:'#ff6b6b'}}>
                  ...
                  <div className="print-card" style={{...card}}>
                    <div style={{...cardHd}}>
                      ...
                      <div style={{display:'flex',gap:5}}>
                        {critical.length>0&&<span style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:'rgba(239,68,68,0.15)',color:'#ff6b6b'}}>{critical.length} critical</span>}
                        {warns.length>0&&<span style={{fontSize:10,padding:'2px 8px',borderRadius:8,background:'rgba(245,158,11,0.15)',color:'#ffc45e'}}>{warns.length} warnings</span>}
                      </div>
                    </div>
                    {issues.length===0?(
                      ...
                      <div key={i} className="dsh-issue" style={{display:'flex',alignItems:'flex-start',gap:12,padding:'10px 16px',borderBottom:`1px solid rgba(255,255,255,0.04)`}}>
                        <div style={{width:22,height:22,borderRadius:6,background:`rgba(${iss.severity==='critical'?'239,68,68':iss.severity==='warn'?'245,158,11':'96,165,250'},0.15)`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:1}}>
                          <AlertTriangle size={11} color={iss.severity==='critical'?'#ff6b6b':iss.severity==='warn'?'#ffc45e':'#60a5fa'}/>
                        </div>
                        <div style={{flex:1}}>
                          <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:2}}>
                            <span style={{fontSize:12,fontWeight:700,color:D.text,fontFamily:'monospace'}}>{iss.type}</span>
                            <span style={{fontSize:10,padding:'1px 6px',borderRadius:8,background:`rgba(${iss.severity==='critical'?'239,68,68':iss.severity==='warn'?'245,158,11':'96,165,250'},0.15)`,color:iss.severity==='critical'?'#ff6b6b':iss.severity==='warn'?'#ffc45e':'#60a5fa'}}>{iss.severity}</span>
                          </div>
                    ...
                    <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                      {['1h','6h','24h','off'].map(iv=>(
                        <button key={iv} onClick={()=>updateInterval(selected.id,iv)}
                          style={{padding:'7px 16px',background:selected.monitor_interval===iv?'rgba(16,185,129,0.15)':'rgba(255,255,255,0.04)',border:`1px solid ${selected.monitor_interval===iv?'rgba(16,185,129,0.4)':'rgba(255,255,255,0.06)'}`,borderRadius:8,color:selected.monitor_interval===iv?'#22d9a0':'rgba(255,255,255,0.45)',fontSize:12,fontWeight:500,cursor:'pointer'}}>
                    ...
                    <div key={i} style={{padding:'16px'}}>
                      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:12}}>
                        {[{l:'Domain',v:cert.domain},{l:'Protocol',v:cert.protocol||'TLS'},{l:'Key size',v:`${cert.key_size||'?'}-bit`},{l:'Chain',v:cert.chain_valid?'✓ Valid':'✗ Invalid'},{l:'CT log',v:cert.ct_log?'✓ Verified':'Not found'},{l:'HSTS',v:cert.hsts||'Not configured'}].map(f=>(
                          <div key={f.l} style={{padding:'10px 12px',background:'rgba(255,255,255,0.03)',borderRadius:8,border:'1px solid rgba(255,255,255,0.06)'}}>
              ...
              {activeTab==='propagation'&&scan?.propagation&&(
                <div style={card}>
                        ...
                        <div key={reg.key} style={{padding:14,background:'rgba(255,255,255,0.03)',borderRadius:10,border:`1px solid ${allPass?'rgba(16,185,129,0.2)':'rgba(245,158,11,0.2)'}`}}>
                          ...
                          {scan.propagation.records?.map(rec=>(
                            <div key={rec.type} style={{display:'flex',alignItems:'center',justifyContent:'space-between',fontSize:11,padding:'3px 0',borderBottom:`1px solid rgba(255,255,255,0.04)`}}>
                              ...
                              <div style={{width:8,height:8,borderRadius:'50%',background:rec[reg.key]==='pass'?'#22d9a0':'#ffc45e'}}/>
                            </div>
                          ))}
                        </div>
                      )
                  ...
                  <div style={{display:'flex',gap:16,padding:'4px 16px 14px',fontSize:11,color:D.muted}}>
                    <span style={{display:'flex',alignItems:'center',gap:5}}><div style={{width:7,height:7,borderRadius:'50%',background:'#22d9a0'}}/> Consistent</span>
                    <span style={{display:'flex',alignItems:'center',gap:5}}><div style={{width:7,height:7,borderRadius:'50%',background:'#ffc45e'}}/> Inconsistent</span>
                  </div>
                </div>
              )}
              ...
              {activeTab==='blacklists'&&scan?.blacklists&&(
                <div style={{display:'flex',flexDirection:'column',gap:12}}>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                    {[{l:'IP address',v:scan.blacklists.ip||'–',c:'#c084fc'},{l:'Lists checked',v:scan.blacklists.results?.length||0,c:'#60a5fa'},{l:'Listed on',v:scan.blacklists.listed_count||0,c:(scan.blacklists.listed_count||0)>0?'#ff6b6b':'#22d9a0'}].map(s=>(
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr'}}>
                      {scan.blacklists.results?.map((bl,i)=>(
                        <div key={bl.name} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 16px',borderBottom:`1px solid rgba(255,255,255,0.04)`,background:bl.listed?'rgba(239,68,68,0.04)':'transparent'}}>
                          <span style={{fontSize:11,fontFamily:'monospace',color:bl.listed?'#ff6b6b':'rgba(255,255,255,0.35)'}}>{bl.name}</span>
                          <div style={{display:'flex',alignItems:'center',gap:5}}>
                            <div style={{width:6,height:6,borderRadius:'50%',background:bl.listed?'#ff6b6b':'#22d9a0'}}/>
                            <span style={{fontSize:10,color:bl.listed?'#ff6b6b':'#22d9a0',fontWeight:500}}>{bl.listed?'Listed':'Clean'}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      <thead>
                        <tr style={{background:'rgba(255,255,255,0.02)'}}>
                          {['Type','Value','TTL','Status'].map(h=>(
                            <th key={h} style={{textAlign:'left',padding:'8px 16px',fontSize:10,fontWeight:600,color:D.muted,textTransform:'uppercase',letterSpacing:'0.06em',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
