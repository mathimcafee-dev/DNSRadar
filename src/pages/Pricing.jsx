...
function Check({ ok, val }) {
  if (typeof ok === 'string') return <span style={{ fontSize:13, color:'#4a5568', fontWeight:500 }}>{ok}</span>
  if (ok === true) return <CheckCircle size={16} color="var(--green)"/>
  return <span style={{ fontSize:13, color:'#d1d5db' }}>—</span>
}

export default function Pricing({ setPage }) {
  return (
    <div style={{ fontFamily:F, background:'#f8fafc', color:'#111', minHeight:'100vh' }}>
      <style>{`
      ...
      {/* Nav */}
      <nav style={{ background:'#ffffff', borderBottom:'1px solid var(--border)', padding:'0 32px', height:56, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' }} onClick={() => setPage('landing')}>
          <div style={{ width:28, height:28, background:'#0073d1', borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center' }}>
            <span style={{ color:'#1a2332', fontSize:14, fontWeight:900 }}>D</span>
          </div>
      ...
      <div style={{ maxWidth:1020, margin:'0 auto', padding:'64px 24px' }}>
        ...
        <div className="fade" style={{ textAlign:'center', marginBottom:60 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#0073d1', textTransform:'uppercase', letterSpacing:'0.12em', marginBottom:12 }}>Pricing</div>
          <h1 style={{ fontSize:'clamp(32px,5vw,52px)', fontWeight:900, letterSpacing:'-0.04em', marginBottom:16 }}>Simple, transparent pricing</h1>
          <p style={{ fontSize:16, color:'#8896a7', maxWidth:440, margin:'0 auto', lineHeight:1.7 }}>
            Free forever for one domain. Upgrade when you need more.<br/>
            <span style={{ fontWeight:600, color:'#0073d1' }}>All plans include a 14-day free trial.</span>
          </p>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:20, marginBottom:64 }}>
          {PLANS.map((plan, i) => (
            <div key={plan.name} className="fade" style={{ animationDelay:`${i*0.08}s`, position:'relative', background:'#ffffff', border: plan.badge ? '2px solid var(--green)' : '1px solid #e5e7eb', borderRadius:16, padding:'28px 26px', display:'flex', flexDirection:'column' }}>
              {plan.badge && (
                <div style={{ position:'absolute', top:-12, left:'50%', transform:'translateX(-50%)', background:'#0073d1', color:'#ffffff', fontSize:11, fontWeight:700, padding:'3px 14px', borderRadius:20, whiteSpace:'nowrap' }}>
                  {plan.badge}
                </div>
              )}
              <div style={{ marginBottom:20 }}>
                <div style={{ fontSize:13, fontWeight:700, color:'#8896a7', textTransform:'uppercase', letterSpacing:'0.09em', marginBottom:8 }}>{plan.name}</div>
                <div style={{ display:'flex', alignItems:'baseline', gap:4, marginBottom:6 }}>
                  <span style={{ fontSize:40, fontWeight:900, letterSpacing:'-0.04em', color:'#111' }}>{plan.price}</span>
                  <span style={{ fontSize:13, color:'#8896a7' }}>/ {plan.period}</span>
                </div>
                <p style={{ fontSize:13, color:'#8896a7', lineHeight:1.6 }}>{plan.desc}</p>
              </div>
              <div style={{ flex:1, marginBottom:24 }}>
                {plan.features.map(f => (
                  <div key={f} style={{ display:'flex', alignItems:'flex-start', gap:9, padding:'6px 0', borderBottom:'1px solid var(--border)' }}>
                    ...
                    <span style={{ fontSize:13, color:'#4a5568', lineHeight:1.5 }}>{f}</span>
                  </div>
                ))}
              </div>
              <button onClick={() => setPage('auth')}
                style={{ width:'100%', padding:'11px', background: plan.ctaStyle==='primary' ? '#0073d1' : '#fff', color: plan.ctaStyle==='primary' ? '#fff' : '#111', border: plan.ctaStyle==='primary' ? 'none' : '1.5px solid #111', borderRadius:9, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:F, transition:'all 0.15s' }}
        ...
        <div style={{ marginBottom:64 }}>
          ...
          <div style={{ background:'#ffffff', border:'1px solid var(--border)', borderRadius:14, overflow:'hidden' }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr style={{ background:'#f8fafc', borderBottom:'1px solid var(--border)' }}>
                  <th style={{ padding:'12px 18px', textAlign:'left', fontSize:12, fontWeight:700, color:'#8896a7', textTransform:'uppercase', letterSpacing:'0.07em' }}>Feature</th>
                  {PLANS.map(p => <th key={p.name} style={{ padding:'12px 18px', fontSize:13, fontWeight:700, color: p.badge ? '#0073d1' : '#111' }}>{p.name}</th>)}
                </tr>
              </thead>
              <tbody>
                {COMPARISON.map((row, i) => (
                  <tr key={row.feature} style={{ borderBottom:'1px solid #f3f4f6', background: i%2===0 ? '#fff' : '#fafafa' }}>
                    <td style={{ padding:'11px 18px', fontSize:13, color:'#4a5568', fontWeight:500 }}>{row.feature}</td>
        ...
        {/* FAQ */}
        <div style={{ textAlign:'center', padding:'48px 32px', background:'#e8f3fc', borderRadius:16, border:'1px solid var(--green-bdr)' }}>
          ...
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={() => setPage('auth')} style={{ padding:'10px 24px', background:'#0073d1', color:'#ffffff', border:'none', borderRadius:9, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:F }}>Start free trial →</button>
            <button onClick={() => setPage('about')} style={{ padding:'10px 20px', background:'transparent', color:'#4a5568', border:'1px solid var(--green-bdr)', borderRadius:9, fontSize:13, cursor:'pointer', fontFamily:F }}>Learn about us</button>
          </div>
        </div>
      </div>
