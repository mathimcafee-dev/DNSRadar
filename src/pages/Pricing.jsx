    ...
    <div style={{ fontFamily:F, background:'var(--card-hi)', color:'#111', minHeight:'100vh' }}>
      ...
      <div style={{ maxWidth:1020, margin:'0 auto', padding:'64px 24px' }}>
        ...
        <div className="fade" style={{ textAlign:'center', marginBottom:60 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'var(--or)', textTransform:'uppercase', letterSpacing:'0.12em', marginBottom:12 }}>Pricing</div>
          ...
          <p style={{ fontSize:16, color:'var(--t3)', maxWidth:440, margin:'0 auto', lineHeight:1.7 }}>
            ...
            <span style={{ fontWeight:600, color:'var(--or)' }}>All plans include a 14-day free trial.</span>
          </p>
        </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:20, marginBottom:64 }}>
          {PLANS.map((plan, i) => (
            <div key={plan.name} className="fade" style={{ animationDelay:`${i*0.08}s`, position:'relative', background:'var(--card)', border: plan.badge ? '2px solid var(--green)' : '1px solid #e5e7eb', borderRadius:16, padding:'28px 26px', display:'flex', flexDirection:'column' }}>
              {plan.badge && (
                <div style={{ position:'absolute', top:-12, left:'50%', transform:'translateX(-50%)', background:'var(--or)', color:'var(--t1)', fontSize:11, fontWeight:700, padding:'3px 14px', borderRadius:20, whiteSpace:'nowrap' }}>
                  {plan.badge}
                </div>
              )}
              ...
              <button onClick={() => setPage('auth')}
                style={{ width:'100%', padding:'11px', background: plan.ctaStyle==='primary' ? 'var(--or)' : '#fff', color: plan.ctaStyle==='primary' ? '#fff' : '#111', border: plan.ctaStyle==='primary' ? 'none' : '1.5px solid #111', borderRadius:9, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:F, transition:'all 0.15s' }}
            ...
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead>
                <tr style={{ background:'var(--card-hi)', borderBottom:'1px solid var(--border)' }}>
                  ...
                  {PLANS.map(p => <th key={p.name} style={{ padding:'12px 18px', fontSize:13, fontWeight:700, color: p.badge ? 'var(--or)' : '#111' }}>{p.name}</th>)}
                </tr>
              </thead>
              <tbody>
        ...
        <div style={{ textAlign:'center', padding:'48px 32px', background:'var(--green-bg)', borderRadius:16, border:'1px solid var(--green-bdr)' }}>
          ...
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={() => setPage('auth')} style={{ padding:'10px 24px', background:'var(--or)', color:'var(--t1)', border:'none', borderRadius:9, fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:F }}>Start free trial →</button>
