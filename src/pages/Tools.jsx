...
const D = {
  bg: '#080c14', surface: '#0d1220', surface2: '#121929',
  border: 'rgba(255,255,255,0.06)', text: '#eef2ff',
  muted: 'rgba(255,255,255,0.45)', dim: 'rgba(255,255,255,0.22)',
}
const card = { background:D.surface, border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, overflow:'hidden' }
const cardHd = { padding:'12px 16px', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between', background:D.surface2 }
...
function CopyBtn({ text }) {
  ...
  return (
    <button onClick={copy} style={{ padding:'5px 12px', background:'rgba(16,185,129,0.15)', border:'1px solid rgba(16,185,129,0.3)', borderRadius:6, color:'#22d9a0', fontSize:12, fontWeight:500, cursor:'pointer', display:'flex', alignItems:'center', gap:5 }}>
        ...
        <div style={{ fontFamily:'monospace', fontSize:12, color:'#7dd3fc', wordBreak:'break-all', lineHeight:1.6 }}>
          {parts.map((p, i) => (
            <span key={i}>
              ...
              <span style={{ color: p.startsWith('v=') ? '#f97316' : p.includes('include:') || p.includes('rua=') || p.includes('ruf=') ? '#c084fc' : '#7dd3fc' }}>{p}</span>
            </span>
          ))}
        </div>
      </div>
        ...
        <div style={{ padding:16 }}>
              ...
              {PROVIDERS.map(p => (
                <label key={p.key} style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 10px', background:providers[p.key] ? 'rgba(16,185,129,0.08)' : 'rgba(255,255,255,0.03)', border:`1px solid ${providers[p.key] ? 'rgba(16,185,129,0.3)' : D.border}`, borderRadius:7, cursor:'pointer', fontSize:12, color:D.text }}>
                  <input type="checkbox" checked={providers[p.key]} onChange={e => setProviders(prev => ({ ...prev, [p.key]: e.target.checked }))}
                    style={{ accentColor:'#22d9a0', width:14, height:14 }}/>
                  {p.label}
                </label>
              ))}
            </div>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <textarea value={customIPs} onChange={e => setCustomIPs(e.target.value)} rows={2} placeholder="192.168.1.1&#10;10.0.0.0/24"
              style={{ width:'100%', padding:'8px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:7, fontSize:12, color:D.text, outline:'none', resize:'vertical', fontFamily:'monospace' }}/>
          </div>
          <div style={{ marginBottom:12 }}>
            ...
            <div style={{ display:'flex', gap:6 }}>
              {[['~all','SoftFail (recommended)'],['−all','HardFail (strict)'],['?all','Neutral']].map(([v, l]) => (
                <button key={v} onClick={() => setQualifier(v)}
                  style={{ padding:'6px 12px', background:qualifier === v ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.04)', border:`1px solid ${qualifier === v ? 'rgba(16,185,129,0.4)' : D.border}`, borderRadius:6, color:qualifier === v ? '#22d9a0' : D.muted, fontSize:11, cursor:'pointer', fontFamily:'monospace' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:12, padding:'8px 12px', background:`rgba(${lookupCount >= 8 ? '239,68,68' : lookupCount >= 6 ? '245,158,11' : '16,185,129'},0.08)`, borderRadius:7, border:`1px solid rgba(${lookupCount >= 8 ? '239,68,68' : lookupCount >= 6 ? '245,158,11' : '16,185,129'},0.2)` }}>
            <AlertTriangle size={13} color={lookupCount >= 8 ? '#ff6b6b' : lookupCount >= 6 ? '#ffc45e' : '#22d9a0'}/>
            <span style={{ fontSize:12, color: lookupCount >= 8 ? '#ff6b6b' : lookupCount >= 6 ? '#ffc45e' : '#22d9a0' }}>
...
function DMARCGenerator({ presetPolicy }) {
  ...
  const record = build()
  const policyColor = policy === 'reject' ? '#22d9a0' : policy === 'quarantine' ? '#ffc45e' : '#ff6b6b'
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
            <div>
              ...
              <input type="email" placeholder={`dmarc@${domain || 'yourdomain.com'}`} value={rua} onChange={e => setRua(e.target.value)}
                style={{ width:'100%', padding:'8px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:7, fontSize:12, color:D.text, outline:'none', fontFamily:'inherit' }}/>
            </div>
            <div>
        ...
        <div style={{ padding:16 }}>
          <div style={{ display:'flex', gap:8, marginBottom:12 }}>
            ...
            <button onClick={scan} disabled={loading || !domain}
              style={{ padding:'8px 20px', background:'rgba(245,158,11,0.15)', border:'1px solid rgba(245,158,11,0.3)', borderRadius:7, color:'#ffc45e', fontSize:13, fontWeight:500, cursor:'pointer', display:'flex', alignItems:'center', gap:6, opacity: !domain ? 0.5 : 1 }}>
              {loading ? <><div style={{ width:13, height:13, border:'2px solid rgba(245,158,11,0.3)', borderTopColor:'#ffc45e', borderRadius:'50%', animation:'spin 0.7s linear infinite' }}/> Scanning {SELECTORS.length} selectors…</> : <><Zap size={13}/> Discover</>}
            </button>
          </div>
                  ...
                  {results.map(r => (
                    <div key={r.selector} style={{ padding:'10px 14px', background:'rgba(245,158,11,0.06)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:8 }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6 }}>
                        ...
                        <span style={{ fontFamily:'monospace', fontSize:13, fontWeight:600, color:'#ffc45e' }}>{r.selector}</span>
                        <span style={{ fontSize:11, color:D.muted }}>._domainkey.{domain}</span>
                        {r.keySize && <span style={{ fontSize:11, padding:'2px 8px', borderRadius:8, background:r.keySize >= 2048 ? 'rgba(16,185,129,0.15)' : 'rgba(245,158,11,0.15)', color:r.keySize >= 2048 ? '#22d9a0' : '#ffc45e' }}>{r.keySize}-bit</span>}
                      </div>
...
function EmailHeaderAnalyser() {
  ...
  const StatusDot = ({ pass, fail, label, value }) => {
    const color = pass ? '#22d9a0' : fail ? '#ff6b6b' : '#ffc45e'
    const status = pass ? 'Pass' : fail ? 'Fail' : 'Unknown'
    return (
      <div style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'10px 14px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
        ...
        <div style={{ padding:16 }}>
          <div style={{ marginBottom:12 }}>
            ...
            <textarea value={header} onChange={e => setHeader(e.target.value)} rows={6}
              ...
              style={{ width:'100%', padding:'10px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:7, fontSize:11, color:D.muted, outline:'none', resize:'vertical', fontFamily:'monospace', lineHeight:1.5 }}/>
          </div>
          <button onClick={analyse} disabled={!header}
            style={{ padding:'8px 20px', background:'rgba(167,139,250,0.15)', border:'1px solid rgba(167,139,250,0.3)', borderRadius:7, color:'#c084fc', fontSize:13, fontWeight:500, cursor:'pointer', display:'flex', alignItems:'center', gap:6, opacity: !header ? 0.5 : 1, marginBottom:12 }}>
          ...
          {result && (
            <div style={{ ...card, border:'1px solid rgba(255,255,255,0.06)' }}>
              ...
              {[
                ...
                <div key={f.l} style={{ display:'flex', gap:12, padding:'8px 14px', borderBottom:'1px solid rgba(255,255,255,0.06)', fontSize:12 }}>
        ...
        <div style={{ padding:16 }}>
          <div style={{ marginBottom:12 }}>
            ...
            <textarea value={input} onChange={e => setInput(e.target.value)} rows={5}
              ...
              style={{ width:'100%', padding:'10px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:7, fontSize:12, color:D.text, outline:'none', resize:'vertical', fontFamily:'monospace' }}/>
          </div>
          <div style={{ display:'flex', gap:8, marginBottom:12 }}>
            ...
            {results.length > 0 && !scanning && (
              <button onClick={exportCSV}
                style={{ padding:'8px 14px', border:'1px solid rgba(255,255,255,0.06)', borderRadius:7, background:'transparent', color:D.muted, fontSize:12, cursor:'pointer', display:'flex', alignItems:'center', gap:5 }}>
                ...
                <thead>
                  <tr style={{ background:'rgba(255,255,255,0.03)' }}>
                    {['Domain','Score','SPF','DMARC','SSL','Critical'].map(h => (
                      <th key={h} style={{ textAlign:'left', padding:'7px 12px', fontSize:10, fontWeight:600, color:D.muted, textTransform:'uppercase', letterSpacing:'0.06em', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {results.map(r => (
                    <tr key={r.domain} style={{ borderBottom:`1px solid rgba(255,255,255,0.04)` }}>
                      ...
                      <td style={{ padding:'8px 12px', fontWeight:700, color: r.score >= 70 ? '#22d9a0' : r.score >= 50 ? '#ffc45e' : r.score ? '#ff6b6b' : D.dim }}>{r.score ?? '–'}</td>
                      {[r.spf, r.dmarc, r.ssl].map((v, i) => {
                        ...
                        return <td key={i} style={{ padding:'8px 12px' }}><span style={{ fontSize:10, padding:'2px 7px', borderRadius:8, background:`rgba(${p ? '16,185,129' : f ? '239,68,68' : '245,158,11'},0.12)`, color: p ? '#22d9a0' : f ? '#ff6b6b' : '#ffc45e' }}>{v ?? '–'}</span></td>
                      })}
                      <td style={{ padding:'8px 12px', color: r.issues > 0 ? '#ff6b6b' : '#22d9a0', fontWeight:600 }}>{r.issues ?? '–'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div style={{ padding:16 }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:12 }}>
          <input value={domain} onChange={e => setDomain(e.target.value)} onKeyDown={e => e.key==='Enter'&&flatten()}
            placeholder="yourdomain.com" style={{ flex:1, padding:'8px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:7, fontSize:13, color:D.text, outline:'none', fontFamily:'inherit' }}/>
          <button onClick={flatten} disabled={loading||!domain.trim()} style={{ padding:'8px 18px', background:'#22d9a0', color:'#fff', border:'none', borderRadius:7, fontSize:12, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:6 }}>
        ...
        {result && (
          <>
            <div style={{ display:'flex', gap:8, marginBottom:10, flexWrap:'wrap' }}>
              {[['IPs resolved', result.ip_count, '#22d9a0'], ['Lookups after', result.lookup_count_after, result.lookup_count_after === 0 ? '#22d9a0' : '#ffc45e'], ['Original lookups', '10 limit', '#6b7280']].map(([l,v,c]) => (
                <div key={l} style={{ background:'rgba(255,255,255,0.04)', borderRadius:8, padding:'8px 12px', border:'1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ fontSize:16, fontWeight:700, color:c }}>{v}</div>
                  <div style={{ fontSize:10, color:D.muted }}>{l}</div>
                </div>
              ))}
            </div>
            {result.warning && <div style={{ fontSize:11, color:'#ffc45e', marginBottom:10, padding:'6px 10px', background:'rgba(245,158,11,0.08)', borderRadius:6 }}>⚠️ {result.warning}</div>}
      ...
      <div style={{ padding:16 }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:14 }}>
          <input value={domain} onChange={e => setDomain(e.target.value)} onKeyDown={e => e.key==='Enter'&&checkSelectors()}
            placeholder="yourdomain.com" style={{ flex:1, padding:'8px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:7, fontSize:13, color:D.text, outline:'none', fontFamily:'inherit' }}/>
          <button onClick={checkSelectors} disabled={checking||!domain.trim()} style={{ padding:'8px 16px', background:'rgba(59,130,246,0.15)', border:'1px solid rgba(59,130,246,0.3)', borderRadius:7, color:'#60a5fa', fontSize:12, fontWeight:600, cursor:'pointer' }}>
        ...
        {selectors.length > 0 && (
          <>
            <div style={{ marginBottom:14 }}>
              ...
              {selectors.map(s => (
                <div key={s.selector} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 12px', background:'rgba(255,255,255,0.03)', borderRadius:8, border:'1px solid rgba(255,255,255,0.06)', marginBottom:6 }}>
                  <code style={{ fontSize:12, color:'#c084fc', fontWeight:600 }}>{s.selector}</code>
                  <span style={{ fontSize:10, padding:'2px 7px', borderRadius:5, background: s.key_size >= 2048 ? 'rgba(16,185,129,0.12)' : 'rgba(245,158,11,0.12)', color: s.key_size >= 2048 ? '#22d9a0' : '#ffc45e', fontWeight:600 }}>{s.key_size || '?'}-bit</span>
                  <span style={{ fontSize:11, color:D.dim }}>TTL {s.ttl}s</span>
                  {s.key_size < 2048 && <span style={{ fontSize:10, color:'#ffc45e', marginLeft:'auto' }}>⚠️ Upgrade to 2048-bit</span>}
                </div>
              ))}
            </div>
      ...
      <div style={{ padding:16 }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:12 }}>
          <input value={domain} onChange={e => setDomain(e.target.value)} onKeyDown={e => e.key==='Enter'&&check()}
            placeholder="yourdomain.com" style={{ flex:1, padding:'8px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:7, fontSize:13, color:D.text, outline:'none', fontFamily:'inherit' }}/>
          <button onClick={check} disabled={loading||!domain.trim()} style={{ padding:'8px 16px', background:'rgba(167,139,250,0.15)', border:'1px solid rgba(167,139,250,0.3)', borderRadius:7, color:'#c084fc', fontSize:12, fontWeight:600, cursor:'pointer' }}>
        ...
        {result && (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
              {[
                ...
                <div key={s.label} style={{ flex:1, minWidth:140, padding:'10px 12px', borderRadius:8, background: s.ok ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)', border:`1px solid ${s.ok ? 'rgba(16,185,129,0.2)' : 'rgba(239,68,68,0.2)'}` }}>
                  ...
                  <div style={{ fontSize:12, fontWeight:600, color: s.ok ? '#22d9a0' : '#ff6b6b' }}>{s.ok ? s.yes : s.no}</div>
                </div>
              ))}
            </div>
            ...
            {result.logo_url && (
              <div style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 12px', background:'rgba(255,255,255,0.03)', borderRadius:8, border:'1px solid rgba(255,255,255,0.06)' }}>
                ...
                <div>
                  ...
                  <a href={result.logo_url} target="_blank" rel="noopener noreferrer" style={{ fontSize:11, color:'#60a5fa', wordBreak:'break-all' }}>{result.logo_url}</a>
                </div>
              </div>
            )}
            {!result.bimi_found && (
...
function MTASTSChecker() {
  ...
  const statusDot = (ok) => <span style={{ display:'inline-block', width:8, height:8, borderRadius:'50%', background: ok ? '#22d9a0' : '#ff6b6b', marginRight:6 }}/>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {[
                ...
                <div key={item.label} style={{ padding:'10px 12px', background:'rgba(255,255,255,0.03)', borderRadius:8, border:'1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  {!item.ok && <div style={{ fontSize:11, color:'#ff6b6b', marginTop:3 }}>Not configured</div>}
                </div>
              ))}
            </div>
    ...
    <div style={card}>
      ...
      <div style={{ padding:16 }}>
        ...
        <textarea value={text} onChange={e => setText(e.target.value)} rows={6} placeholder={'example.com\ngoogle.com\nhttps://microsoft.com/\nyourdomain.org'}
          style={{ width:'100%', padding:'10px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:8, fontSize:12, fontFamily:'monospace', color:D.text, outline:'none', resize:'vertical', boxSizing:'border-box', marginBottom:10 }}/>
        {!user && <div style={{ fontSize:11, color:'#ffc45e', marginBottom:10 }}>⚠️ Sign in to import domains to your account</div>}
        <button onClick={importDomains} disabled={importing || !text.trim() || !user} style={{ padding:'8px 18px', background:'#22d9a0', color:'#fff', border:'none', borderRadius:7, fontSize:12, fontWeight:600, cursor:'pointer', marginBottom: results.length ? 14 : 0 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:4 }}>
            {results.map(r => (
              <div key={r.domain} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 10px', background:`rgba(${r.ok ? '16,185,129' : '239,68,68'},0.06)`, borderRadius:6, border:`1px solid rgba(${r.ok ? '16,185,129' : '239,68,68'},0.15)` }}>
                <span style={{ fontSize:11, color: r.ok ? '#22d9a0' : '#ffc45e' }}>{r.ok ? '✓' : '—'}</span>
            ...
            {done && <div style={{ fontSize:12, color:'#22d9a0', marginTop:6 }}>Done. Go to Dashboard to verify and scan your new domains.</div>}
          </div>
        )}
      </div>
    </div>
...
export default function Tools({ user }) {
  ...
  return (
    <div style={{ background:D.bg, minHeight:'100%', fontFamily:"'DM Sans','Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ padding:'14px 20px', borderBottom:'1px solid rgba(255,255,255,0.06)', background:D.surface }}>
        <h2 style={{ fontSize:16, fontWeight:700, color:D.text, marginBottom:12 }}>Tools</h2>
        <div style={{ display:'flex', gap:2, flexWrap:'wrap' }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              style={{ padding:'7px 16px', background:activeTab===t.id?'rgba(16,185,129,0.12)':'transparent', border:`1px solid ${activeTab===t.id?'rgba(16,185,129,0.25)':'transparent'}`, borderRadius:8, color:activeTab===t.id?'#22d9a0':'rgba(255,255,255,0.45)', fontSize:12, fontWeight:500, cursor:'pointer', display:'flex', alignItems:'center', gap:5 }}>
              {t.label}
            </button>
          ))}
        </div>
