...
export default function Reports({ user }) {
  ...
  const pieData = [
    { name:'Compliant', value:totalPass, color:'#10b981' },
    { name:'Failing', value:totalFail, color:'#ef4444' },
  ].filter(d=>d.value>0)
    ...
    <div style={{ background:D.bg, minHeight:'100%', padding:20, fontFamily:"'DM Sans','Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          ...
          <button onClick={()=>setUploadXML(u=>!u)}
            style={{ padding:'7px 14px', background:'rgba(16,185,129,0.15)', border:'1px solid rgba(16,185,129,0.3)', borderRadius:8, color:'#10b981', fontSize:12, fontWeight:500, cursor:'pointer', display:'flex', alignItems:'center', gap:5 }}>
      ...
      {loading ? (
        ...
        <div style={{ ...card, padding:'48px', textAlign:'center' }}>
          ...
          <div style={{ fontFamily:'monospace', fontSize:12, color:'#10b981', background:'rgba(16,185,129,0.08)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:8, padding:'10px 16px', margin:'12px auto', maxWidth:480, textAlign:'left' }}>
        ...
        <>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:14 }}>
            {[
              ...
              { l:'Compliance rate', v:`${complianceRate}%`, c:complianceRate>=90?'#10b981':complianceRate>=70?'#f59e0b':'#ef4444', sub:`${totalPass.toLocaleString()} passing` },
              { l:'Failing', v:totalFail.toLocaleString(), c:totalFail>0?'#ef4444':'#10b981', sub:'emails failing DMARC' },
              { l:'Threats detected', v:threats.length, c:threats.length>0?'#ef4444':'#10b981', sub:'unauthorised sources' },
            ].map(k=>(
                  ...
                  {serviceList.map((s,i)=>{
                    ...
                    return (
                      <tr key={i} style={{ borderBottom:`1px solid rgba(255,255,255,0.04)` }}>
                        ...
                        <td style={{ padding:'9px 14px', color:s.fail>0?'#ef4444':D.d, fontFamily:'monospace' }}>{s.fail.toLocaleString()}</td>
                        <td style={{ padding:'9px 14px' }}>
                          <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                            <div style={{ height:5, width:60, background:'rgba(255,255,255,0.06)', borderRadius:3, overflow:'hidden' }}>
                              <div style={{ height:'100%', width:`${rate}%`, background:rate>=90?'#10b981':rate>=70?'#f59e0b':'#ef4444', borderRadius:3 }}/>
                            </div>
                            <span style={{ fontSize:11, color:rate>=90?'#10b981':rate>=70?'#f59e0b':'#ef4444', fontWeight:600 }}>{rate}%</span>
                          </div>
                        </td>
                        <td style={{ padding:'9px 14px' }}>
                          <span style={{ fontSize:10, padding:'2px 7px', borderRadius:8, background:`rgba(${s.fail>0&&rate<50?'239,68,68':'16,185,129'},0.15)`, color:s.fail>0&&rate<50?'#ef4444':'#10b981' }}>
            ...
            {reports.map(r=>(
              <div key={r.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'10px 16px', borderBottom:`1px solid rgba(255,255,255,0.04)` }}>
                ...
                <div style={{ textAlign:'right' }}>
                  ...
                  <div style={{ fontSize:10, color:r.fail_count>0?'#ef4444':'#10b981' }}>
                    {r.pass_count||0} pass · {r.fail_count||0} fail
                  </div>
                </div>
                <div style={{ width:48, textAlign:'right', fontSize:12, fontWeight:700, color:r.total_volume>0?Math.round((r.pass_count/r.total_volume)*100)>=90?'#10b981':'#f59e0b':D.d }}>
