...
function CopyBtn({ text }) {
  ...
  return (
    <button onClick={() => { navigator.clipboard.writeText(text); setC(true); setTimeout(()=>setC(false),2000) }}
      style={{ padding:'4px 10px', background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.25)', borderRadius:5, color:'#00d97e', fontSize:10, cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}>
          ...
          <div style={card}>
            ...
            <div style={{ padding:16 }}>
              <div style={{ fontSize:12, color:D.muted, marginBottom:12, lineHeight:1.6 }}>
                Use API keys to access DomainRadar programmatically. Include as <code style={{ fontFamily:'monospace', background:'#141b2d', padding:'1px 6px', borderRadius:4 }}>Authorization: Bearer YOUR_KEY</code> header.
              </div>
              {/* Endpoint docs */}
              ...
              {createdKey && (
                <div style={{ padding:'12px 14px', background:'rgba(16,185,129,0.06)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:8, marginBottom:16 }}>
                  <div style={{ fontSize:12, fontWeight:600, color:'#00d97e', marginBottom:6 }}>✓ Key created — copy it now, it won't be shown again</div>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    <code style={{ flex:1, fontFamily:'monospace', fontSize:12, color:'#00d97e', background:'rgba(0,0,0,0.25)', padding:'6px 10px', borderRadius:6, wordBreak:'break-all' }}>{createdKey.key}</code>
              ...
              {apiKeys.map(k => (
                <div key={k.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 0', borderBottom:`1px solid #f9fafb` }}>
                  <Key size={13} color={k.revoked?D.dim:'#00d97e'}/>
                  <div style={{ flex:1 }}>
                  ...
                  {k.revoked ? <span style={{ fontSize:10, padding:'2px 8px', borderRadius:8, background:'#141b2d', color:D.dim }}>Revoked</span>
                    : <button onClick={() => revokeKey(k.id)} style={{ padding:'4px 10px', background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:6, color:'#ff5e5e', fontSize:11, cursor:'pointer', display:'flex', alignItems:'center', gap:4 }}><Trash2 size={10}/> Revoke</button>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={card}>
          ...
          <div style={{ padding:16 }}>
            <div style={{ marginBottom:12 }}>
              ...
              <div style={{ padding:'8px 12px', background:'#0f1525', borderRadius:7, fontSize:13, color:D.dim, fontFamily:'monospace' }}>{user?.email}</div>
            </div>
            ...
            <div style={{ display:'flex', gap:8 }}>
              ...
              <button onClick={() => { if(window.confirm('Sign out?')) signOut() }}
                style={{ padding:'8px 14px', background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:7, color:'#ff5e5e', fontSize:13, cursor:'pointer', display:'flex', alignItems:'center', gap:5 }}>
        ...
        <div style={card}>
          ...
          <div style={{ padding:16 }}>
            <label style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16, cursor:'pointer' }}>
              <input type="checkbox" checked={profile.alert_email||true} onChange={e => setProfile(p => ({ ...p, alert_email: e.target.checked }))} style={{ accentColor:'#00d97e', width:16, height:16 }}/>
              <div>
...
function TeamSection({ user }) {
  ...
  const statusColor = { pending:'#ffb547', active:'#00d97e', rejected:'#ff5e5e' }
  const roleColor   = { admin:'#a78bfa', editor:'#3b82f6', viewer:'rgba(255,255,255,0.45)' }
    ...
    <div style={card}>
      ...
      <div style={{ padding:16 }}>
        <div style={{ display:'flex', gap:8, marginBottom:16, flexWrap:'wrap' }}>
          ...
          <button onClick={invite} disabled={saving||!email.trim()} style={{ padding:'8px 16px', background:'#00d97e', color:'#fff', border:'none', borderRadius:7, fontSize:12, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap' }}>
            {saving ? 'Sending…' : 'Invite'}
          </button>
        </div>
        {msg && <div style={{ fontSize:12, color:'#00d97e', marginBottom:12, padding:'6px 10px', background:'rgba(16,185,129,0.08)', borderRadius:6 }}>{msg}</div>}
        {members.length === 0 ? (
          ...
          <div key={m.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:'#0f1525', borderRadius:8, border:`1px solid ${D.border}`, marginBottom:6 }}>
            ...
            <span style={{ fontSize:10, padding:'2px 7px', borderRadius:5, background:`${roleColor[m.role]||'rgba(255,255,255,0.45)'}18`, color:roleColor[m.role]||'rgba(255,255,255,0.45)', fontWeight:600, border:`1px solid ${roleColor[m.role]||'rgba(255,255,255,0.45)'}30` }}>{m.role}</span>
            <span style={{ fontSize:10, padding:'2px 7px', borderRadius:5, background:`${statusColor[m.status]||'rgba(255,255,255,0.45)'}18`, color:statusColor[m.status]||'rgba(255,255,255,0.45)', fontWeight:600 }}>{m.status}</span>
