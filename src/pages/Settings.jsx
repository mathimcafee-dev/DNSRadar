...
const card = { background:D.surface, border:'1px solid #1e2535', borderRadius:12, overflow:'hidden', marginBottom:16 }
const cardHd = { padding:'12px 16px', borderBottom:'1px solid #1e2535', background:D.surface2, fontSize:12, fontWeight:600, color:D.text, display:'flex', alignItems:'center', gap:7 }
...
export default function Settings({ user }) {
  ...
  return (
    <div style={{ background:D.bg, minHeight:'100%', padding:20, fontFamily:"'DM Sans','Inter',system-ui,sans-serif", maxWidth:800 }}>
      ...
      {/* Tabs */}
      <div style={{ display:'flex', gap:0, borderBottom:'1px solid #1e2535', marginBottom:20 }}>
          ...
          <div style={card}>
            ...
            <div style={{ padding:16 }}>
              <div style={{ fontSize:12, color:D.muted, marginBottom:12, lineHeight:1.6 }}>
                Use API keys to access DomainRadar programmatically. Include as <code style={{ fontFamily:'monospace', background:'#1e2535', padding:'1px 6px', borderRadius:4 }}>Authorization: Bearer YOUR_KEY</code> header.
              </div>
              {/* Endpoint docs */}
              ...
              {apiKeys.map(k => (
                <div key={k.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'9px 0', borderBottom:`1px solid rgba(255,255,255,0.04)` }}>
                  ...
                  {k.revoked ? <span style={{ fontSize:10, padding:'2px 8px', borderRadius:8, background:'#1e2535', color:D.dim }}>Revoked</span>
          ...
          <div style={{ padding:16 }}>
            ...
            <div style={{ marginBottom:16 }}>
              ...
              <input value={profile.alert_webhook||''} onChange={e => setProfile(p => ({ ...p, alert_webhook: e.target.value }))}
                ...
                style={{ width:'100%', padding:'8px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid #1e2535', borderRadius:7, fontSize:13, color:D.text, outline:'none', fontFamily:'monospace' }}/>
            </div>
            <button onClick={saveProfile} disabled={saving}
              style={{ padding:'8px 18px', background:'#00e5a0', color:'#fff', border:'none', borderRadius:7, fontSize:13, fontWeight:500, cursor:'pointer' }}>
              {saving ? 'Saving…' : 'Save'}
...
function TeamSection({ user }) {
  ...
  const D = { surface:'#161b23', surface2:'#1d2330', border:'#1e2535', text:'#f0f4ff', muted:'#8993ac', dim:'#4a5470' }
  ...
  const input = { width:'100%', padding:'8px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid #1e2535', borderRadius:7, fontSize:13, color:D.text, outline:'none', fontFamily:'inherit', boxSizing:'border-box' }
    ...
    <div style={card}>
      ...
      <div style={{ padding:16 }}>
        ...
        {members.length === 0 ? (
          ...
          <div key={m.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:'rgba(255,255,255,0.03)', borderRadius:8, border:'1px solid #1e2535', marginBottom:6 }}>
