...
export default function Landing({ setPage, setScanDomain, setScanType }) {
  ...
  const navBtn = { padding:'7px 14px', background:'transparent', color:'rgba(255,241,236,0.45)', border:'1px solid rgba(255,107,43,0.12)', borderRadius:8, fontSize:13, fontWeight:500, cursor:'pointer', fontFamily:F, transition:'all 0.15s' }
  ...
  return (
    <div style={{ fontFamily:F, background:'var(--page)', color:'var(--t1)', lineHeight:1.6, overflowX:'hidden' }}>
      <style>{`
        ...
        .nl{font-size:13px;color:rgba(255,241,236,0.4);cursor:pointer;padding:5px 12px;border-radius:7px;font-family:${F};background:none;border:none;transition:all 0.15s;font-weight:500}
        .nl:hover{color:var(--t1);background:rgba(255,107,43,0.08)}
        .si:focus{border-color:var(--or-bdr)!important;box-shadow:0 0 0 3px rgba(255,107,43,0.1)!important;outline:none}
      `}</style>

      {/* NAV */}
      <nav style={{ position:'fixed',top:0,left:0,right:0,zIndex:200,height:60,display:'flex',alignItems:'center',padding:'0 40px',justifyContent:'space-between', background:scrolled?'rgba(12,9,7,0.95)':'transparent', backdropFilter:scrolled?'blur(16px)':'none', borderBottom:scrolled?'1px solid var(--border)':'1px solid transparent', transition:'all 0.2s' }}>
        <div style={{ display:'flex',alignItems:'center',gap:10,cursor:'pointer' }} onClick={()=>window.scrollTo({top:0,behavior:'smooth'})}>
          ...
          <span style={{ fontSize:16,fontWeight:900,letterSpacing:'-.03em',color:'var(--t1)' }}>Domain<span style={{ color:'var(--or)' }}>Radar</span></span>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:4 }}>
          ...
          <button onClick={()=>setPage('auth')} style={{ padding:'8px 18px',background:'var(--or)',color:'#0c0907',border:'none',borderRadius:8,fontSize:13,fontWeight:800,cursor:'pointer',fontFamily:F,transition:'all 0.15s' }}
      ...
      <section style={{ minHeight:'100vh',display:'grid',gridTemplateColumns:'1fr 1fr',alignItems:'center',padding:'120px 60px 80px',gap:48,maxWidth:1200,margin:'0 auto' }}>
        <div>
          <div className="fu" style={{ animationDelay:'0.05s',marginBottom:20 }}>
            <span style={{ display:'inline-flex',alignItems:'center',gap:8,background:'var(--or-bg)',color:'var(--or)',border:'1px solid var(--or-bdr)',borderRadius:24,padding:'6px 16px',fontSize:12,fontWeight:700 }}>
          ...
          <h1 className="fu" style={{ animationDelay:'0.1s',fontSize:'clamp(36px,4.5vw,60px)',fontWeight:900,letterSpacing:'-.04em',lineHeight:1.04,marginBottom:20,color:'var(--t1)' }}>
            Your domain's<br/><span style={{ color:'var(--or)' }}>security,</span><br/>audited and<br/><span style={{ color:'var(--cy)' }}>fixed.</span>
          </h1>
          <p className="fu" style={{ animationDelay:'0.15s',fontSize:16,color:'var(--t2)',maxWidth:460,marginBottom:36,lineHeight:1.75 }}>
            ...
            <form onSubmit={scan} style={{ display:'flex',gap:10,marginBottom:10 }}>
              <div style={{ flex:1,position:'relative' }}>
                ...
                <input value={domain} onChange={e=>setDomain(e.target.value)} placeholder="yourdomain.com" className="si"
                  style={{ paddingLeft:38,fontSize:14,fontFamily:MONO,background:'rgba(255,255,255,0.04)',border:'1px solid var(--or-bdr)',borderRadius:10 }}/>
              </div>
              <button type="submit" style={{ padding:'0 24px',background:'var(--cy)',color:'#0c0907',border:'none',borderRadius:10,fontSize:14,fontWeight:800,cursor:'pointer',fontFamily:F,whiteSpace:'nowrap',transition:'all 0.15s' }}
                onMouseEnter={e=>e.currentTarget.style.filter='brightness(1.1)'}
                onMouseLeave={e=>e.currentTarget.style.filter='none'}>
                Scan →
              </button>
        ...
        <div className="fu" style={{ animationDelay:'0.25s',display:'grid',gridTemplateColumns:'1fr 1fr',gap:12 }}>
          {[
            ...
            <div key={s.l} style={{ background:s.bg,borderRadius:16,padding:'24px 22px' }}>
              <div style={{ fontSize:42,fontWeight:900,letterSpacing:'-.04em',color:s.dark?'#0c0907':'#fff',lineHeight:1 }}>{s.v}</div>
              <div style={{ fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'.09em',marginTop:8,color:s.dark?'rgba(12,9,7,0.55)':'rgba(255,255,255,0.6)' }}>{s.l}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* FEATURES */}
      <section id="features" style={{ padding:'100px 60px',background:'rgba(255,107,43,0.02)',borderTop:'1px solid var(--border)',maxWidth:'100%' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:60 }}>
            <div style={{ fontSize:11,fontWeight:700,color:'var(--or)',textTransform:'uppercase',letterSpacing:'.14em',marginBottom:12 }}>What we check</div>
            <h2 style={{ fontSize:'clamp(28px,4vw,44px)',fontWeight:900,letterSpacing:'-.03em',color:'var(--t1)',marginBottom:14 }}>Everything your domain needs</h2>
            <p style={{ fontSize:15,color:'var(--t2)',maxWidth:500,margin:'0 auto' }}>Nine categories. All live, all in parallel, all explained in plain English.</p>
          </div>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:14 }}>
            {FEATURES.map(f=>(
              <div key={f.label} className="hc" style={{ background:'var(--card)',border:`1px solid ${f.bd}`,borderRadius:16,padding:'22px 24px' }}>
      ...
      {/* HOW IT WORKS */}
      <section id="how" style={{ padding:'100px 60px',borderTop:'1px solid var(--border)',maxWidth:'100%' }}>
        <div style={{ maxWidth:1000,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:60 }}>
            <div style={{ fontSize:11,fontWeight:700,color:'var(--cy)',textTransform:'uppercase',letterSpacing:'.14em',marginBottom:12 }}>Process</div>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:36 }}>
            {STEPS.map(s=>(
              <div key={s.n}>
                <div style={{ fontSize:11,fontWeight:800,color:s.c,fontFamily:MONO,marginBottom:12,letterSpacing:'.08em' }}>{s.n}</div>
                <div style={{ width:44,height:4,background:s.c,borderRadius:2,marginBottom:16,opacity:0.7 }}/>
                <div style={{ fontSize:16,fontWeight:800,color:'var(--t1)',marginBottom:10 }}>{s.title}</div>
      ...
      {/* FAQ */}
      <section style={{ padding:'80px 60px',background:'rgba(255,107,43,0.02)',borderTop:'1px solid var(--border)',maxWidth:'100%' }}>
        <div style={{ maxWidth:700,margin:'0 auto' }}>
          ...
          <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
            {FAQS.map((f,i)=>(
              <div key={i} style={{ background:'var(--card)',border:`1px solid ${openFaq===i?'var(--or-bdr)':'var(--border)'}`,borderRadius:12,overflow:'hidden',transition:'border-color 0.2s' }}>
      ...
      {/* CTA */}
      <section style={{ padding:'100px 60px',textAlign:'center',borderTop:'1px solid var(--border)' }}>
        <div style={{ maxWidth:560,margin:'0 auto' }}>
          <h2 style={{ fontSize:'clamp(32px,5vw,54px)',fontWeight:900,color:'var(--t1)',letterSpacing:'-.04em',lineHeight:1.08,marginBottom:18 }}>
            Scan your domain.<br/><span style={{ color:'var(--cy)' }}>Free, right now.</span>
          </h2>
          ...
          <div style={{ display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap' }}>
            <button onClick={()=>{window.scrollTo({top:0,behavior:'smooth'});setTimeout(()=>document.querySelector('.si')?.focus(),400)}}
              style={{ padding:'14px 36px',background:'var(--or)',color:'#0c0907',border:'none',borderRadius:10,fontSize:15,fontWeight:800,cursor:'pointer',fontFamily:F,transition:'all 0.15s' }}
              onMouseEnter={e=>e.currentTarget.style.filter='brightness(1.1)'}
              onMouseLeave={e=>e.currentTarget.style.filter='none'}>
              Audit your domain →
            </button>
      ...
      {/* FOOTER */}
      <footer style={{ background:'rgba(0,0,0,0.3)',borderTop:'1px solid var(--border)',padding:'28px 60px' }}>
        <div style={{ maxWidth:1200,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:16 }}>
          <div style={{ display:'flex',alignItems:'center',gap:10 }}>
            ...
            <span style={{ fontSize:13,color:'var(--t2)',fontWeight:700 }}>DomainRadar</span>
