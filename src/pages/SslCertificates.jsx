...
const D = { bg:'#0f1117', surface:'#0f1117', surface2:'#1c2028', border:'rgba(255,255,255,0.08)', text:'#f1f5ff', muted:'#8b95b0', dim:'rgba(255,255,255,0.28)' }
const card = { background:D.surface, border:'1px solid rgba(255,255,255,0.08)', borderRadius:12, overflow:'hidden' }

function daysColor(days) {
  if (days == null) return '#6b7280'
  if (days <= 7)  return '#ff5757'
  if (days <= 30) return '#ffad30'
  if (days <= 60) return '#4d9cff'
  return '#10e898'
}
...
function Field({ label, value, good }) {
  const color = good === true ? '#10e898' : good === false ? '#ff5757' : D.muted
  return (
    <div>
    ...
    <div style={{ ...card, marginBottom:10 }}>
      <div style={{ padding:'12px 16px', display:'flex', alignItems:'center', gap:12, cursor:'pointer', flexWrap:'wrap' }} onClick={onToggle}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
          ...
          {cert.chain_valid === false && <span style={{ fontSize:10, padding:'2px 7px', borderRadius:6, background:'rgba(239,68,68,0.12)', color:'#ff5757', border:'1px solid rgba(239,68,68,0.2)', fontWeight:500 }}>Chain error</span>}
          {cert.weak_cipher_detected && <span style={{ fontSize:10, padding:'2px 7px', borderRadius:6, background:'rgba(245,158,11,0.12)', color:'#ffad30', border:'1px solid rgba(245,158,11,0.2)', fontWeight:500 }}>Weak cipher</span>}
          ...
          {tlsVersions.length > 0 && (
            <div style={{ marginBottom:12 }}>
              ...
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                {tlsVersions.map(v => (
                  <span key={v} style={{ fontSize:11, padding:'2px 8px', borderRadius:6, background:(v==='TLSv1.0'||v==='TLSv1.1')?'rgba(239,68,68,0.12)':'rgba(16,185,129,0.12)', color:(v==='TLSv1.0'||v==='TLSv1.1')?'#ff5757':'#10e898', border:`1px solid ${(v==='TLSv1.0'||v==='TLSv1.1')?'rgba(239,68,68,0.2)':'rgba(16,185,129,0.2)'}`, fontWeight:500 }}>{v}</span>
                ))}
              </div>
            </div>
          )}
            ...
            <div style={{ marginBottom:10 }}>
              ...
              <div style={{ display:'flex', gap:5, flexWrap:'wrap' }}>
                {sans.slice(0,12).map(s => (
                  <span key={s} style={{ fontSize:10, fontFamily:'monospace', padding:'2px 7px', borderRadius:5, background:'rgba(255,255,255,0.04)', color:D.muted, border:'1px solid rgba(255,255,255,0.08)' }}>{s}</span>
                ))}
...
function SummaryCard({ label, value, color, onClick }) {
  return (
    <div onClick={onClick} style={{ background:D.surface, border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, padding:'12px 14px', cursor:onClick?'pointer':'default' }}>
    ...
    <div style={{ maxWidth:940, margin:'0 auto', padding:'24px 20px', fontFamily:"'DM Sans','Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:20, flexWrap:'wrap', gap:12 }}>
        ...
        <button onClick={scanAll} disabled={scanning || !domains.some(d=>d.verified)}
          style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 14px', background:'rgba(16,185,129,0.12)', border:'1px solid rgba(16,185,129,0.25)', borderRadius:8, color:'#10e898', fontSize:12, fontWeight:600, cursor:'pointer', opacity:scanning?0.6:1 }}>
      ...
      <div style={{ ...card, marginBottom:20, padding:'14px 16px' }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          <input
            ...
            style={{ flex:1, padding:'9px 12px', background:'rgba(0,0,0,0.3)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:8, fontSize:13, color:D.text, outline:'none', fontFamily:'inherit' }}
          />
          <button
            onClick={scanManualDomain}
            disabled={manualScanning || !manualDomain.trim()}
            style={{ padding:'9px 18px', background:'#10e898', color:'#fff', border:'none', borderRadius:8, fontSize:13, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', gap:7, opacity: manualScanning || !manualDomain.trim() ? 0.6 : 1 }}>
            {manualScanning
      ...
      {/* Status messages */}
      {scanMsg && <div style={{ padding:'9px 14px', background:'rgba(16,185,129,0.08)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:8, color:'#10e898', fontSize:12, marginBottom:14 }}>{scanMsg}</div>}
      {scanError && <div style={{ padding:'9px 14px', background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:8, color:'#ff5757', fontSize:12, marginBottom:14 }}>{scanError}</div>}

      {/* Summary cards */}
      {certs.length > 0 && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(140px,1fr))', gap:10, marginBottom:18 }}>
          <SummaryCard label="Total" value={certs.length} color="#6b7280"/>
          <SummaryCard label="Expiring ≤ 30d" value={expiringSoon} color={expiringSoon>0?'#ffad30':'#10e898'} onClick={()=>setFilter('expiring')}/>
          <SummaryCard label="Issues" value={issues} color={issues>0?'#ff5757':'#10e898'} onClick={()=>setFilter('issues')}/>
      ...
      {certs.length > 0 && (
        <div style={{ display:'flex', gap:6, marginBottom:14, flexWrap:'wrap' }}>
          {[['all','All'],['expiring','Expiring soon'],['issues','Issues']].map(([v,l])=>(
            <button key={v} onClick={()=>setFilter(v)} style={{ padding:'5px 12px', background:filter===v?'rgba(16,185,129,0.12)':'rgba(255,255,255,0.04)', border:`1px solid ${filter===v?'rgba(16,185,129,0.25)':'rgba(255,255,255,0.08)'}`, borderRadius:7, color:filter===v?'#10e898':D.muted, fontSize:12, fontWeight:filter===v?600:400, cursor:'pointer' }}>{l}</button>
          ))}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ ...card, padding:'48px 24px', textAlign:'center' }}>
          ...
          {domains.filter(d=>d.verified).length > 0 && (
            <button onClick={scanAll} disabled={scanning} style={{ padding:'8px 20px', background:'#10e898', border:'none', borderRadius:8, color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer' }}>
      ...
      {!loading && domains.filter(d=>d.verified&&!certs.find(c=>c.domain_name===d.domain_name)).length > 0 && (
        <div style={{ marginTop:20 }}>
          ...
          {domains.filter(d=>d.verified&&!certs.find(c=>c.domain_name===d.domain_name)).map(d=>(
            <div key={d.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'10px 14px', background:D.surface, border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, marginBottom:6 }}>
              <span style={{ fontSize:13, color:D.muted }}>{d.domain_name}</span>
              <button onClick={()=>scanDomain(d)} disabled={scanning} style={{ padding:'5px 14px', background:'rgba(255,255,255,0.08)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:6, color:D.muted, fontSize:11, fontWeight:500, cursor:'pointer' }}>Scan now</button>
            </div>
          ))}
        </div>
      )}
