...
const S = {
  ...
  name:   { fontSize:14, fontWeight:800, color:'#eef2ff', letterSpacing:'-0.03em', fontFamily:"'Syne',system-ui,sans-serif" },
  beta:   { fontSize:9, color:'#22d9a0', fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', marginTop:-1 },
}
    ...
    return (
      <button key={item.id} onClick={() => setPage(item.id)} title={collapsed ? item.label : undefined}
        style={{
          ...
          borderRadius: 9,
          color: active ? '#22d9a0' : 'rgba(255,255,255,0.4)',
        ...
        {active && !collapsed && <span style={{ position:'absolute', left:0, top:'18%', bottom:'18%', width:3, borderRadius:'0 2px 2px 0', background:'#22d9a0', boxShadow:'0 0 8px rgba(34,217,160,0.7)' }}/>}
        <item.icon size={15} style={{ flexShrink:0, color: active ? '#22d9a0' : 'rgba(255,255,255,0.3)', transition:'color 0.12s' }}/>
        {!collapsed && <span style={{ flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item.label}</span>}
        {hasBadge && !collapsed && <span style={{ fontSize:10, padding:'1px 6px', borderRadius:10, background:'rgba(255,107,107,0.15)', color:'#ff6b6b', fontWeight:700, border:'1px solid rgba(255,107,107,0.2)' }}>{alertCount > 99 ? '99+' : alertCount}</span>}
        {hasBadge && collapsed && <span style={{ position:'absolute', top:6, right:9, width:6, height:6, borderRadius:'50%', background:'#ff6b6b' }}/>}
      </button>
    )
  }

  return (
    <aside style={{ width:w, minWidth:w, height:'100vh', position:'sticky', top:0, background:'#0d1220', borderRight:'1px solid rgba(255,255,255,0.06)', display:'flex', flexDirection:'column', transition:'width 0.2s cubic-bezier(.4,0,.2,1)', overflow:'hidden', zIndex:100, flexShrink:0 }}>

      {/* Logo row */}
      <div style={{ height:56, padding:collapsed?'0':'0 10px 0 16px', display:'flex', alignItems:'center', justifyContent:collapsed?'center':'space-between', borderBottom:'1px solid rgba(255,255,255,0.06)', flexShrink:0 }}>
        {!collapsed && (
      ...
      <nav style={{ flex:1, overflowY:'auto', padding:'8px 0', scrollbarWidth:'none' }}>
        ...
        {!user && (
          <div style={{ padding:'16px 8px', display:'flex', flexDirection:'column', gap:6 }}>
            <button onClick={() => setPage('auth')} style={{ padding:'9px', background:'#22d9a0', color:'#021a10', border:'none', borderRadius:9, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:"'DM Sans',system-ui,sans-serif" }}>Start free</button>
      ...
      {user && (
        <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', padding:collapsed?'6px 0':'6px', flexShrink:0 }}>
          <button onClick={() => setPage('settings')} title={collapsed?'Settings':undefined}
            ...
            onMouseLeave={e => { if(page!=='settings'){e.currentTarget.style.background='transparent';e.currentTarget.style.color='rgba(255,255,255,0.4)'}}}>
            <Settings size={15} style={{ color:page==='settings'?'#22d9a0':'rgba(255,255,255,0.3)', flexShrink:0 }}/>
            {!collapsed && 'Settings'}
          </button>

          <div style={{ position:'relative' }}>
            <button onClick={() => setUserOpen(o=>!o)}
              style={{ width:'100%', display:'flex', alignItems:'center', gap:9, padding:collapsed?'7px 0':'8px 10px', justifyContent:collapsed?'center':'flex-start', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:9, cursor:'pointer', transition:'background 0.12s' }}
              onMouseEnter={e => e.currentTarget.style.background='rgba(255,255,255,0.08)'}
              onMouseLeave={e => e.currentTarget.style.background='rgba(255,255,255,0.04)'}>
              <div style={{ width:26, height:26, borderRadius:'50%', background:'linear-gradient(135deg,rgba(34,217,160,0.3),rgba(34,217,160,0.1))', color:'#22d9a0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, flexShrink:0, border:'1px solid rgba(34,217,160,0.2)', fontFamily:"'DM Sans',system-ui,sans-serif" }}>
                {user.email?.[0]?.toUpperCase()}
              </div>
              {!collapsed && (
                <>
                  <div style={{ flex:1, minWidth:0, textAlign:'left' }}>
                    <div style={{ fontSize:12, fontWeight:600, color:'#eef2ff', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', letterSpacing:'-0.02em' }}>{user.email?.split('@')[0]}</div>
                    <div style={{ fontSize:10, color:'rgba(255,255,255,0.25)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{user.email}</div>
                  </div>
            ...
            {userOpen && (
              <div style={{ position:'absolute', bottom:'calc(100% + 6px)', left:0, right:0, background:'#121929', border:'1px solid rgba(255,255,255,0.1)', borderRadius:10, boxShadow:'0 -12px 32px rgba(0,0,0,0.6)', padding:'4px', zIndex:200, minWidth:180 }}>
                <div style={{ padding:'8px 10px 6px', fontSize:11, color:'rgba(255,255,255,0.25)', borderBottom:'1px solid rgba(255,255,255,0.06)', marginBottom:4 }}>{user.email}</div>
                {[
                  { label:'Settings', icon:Settings, action:() => { setPage('settings'); setUserOpen(false) }, color:'rgba(255,255,255,0.7)' },
                  { label:'Sign out', icon:LogOut, action:() => { signOut(); setUserOpen(false) }, color:'#ff6b6b' },
                ].map(item => (
