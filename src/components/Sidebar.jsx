  ...
  return (
    <aside style={{ width:w, minWidth:w, height:'100vh', position:'sticky', top:0, background:'#111827', borderRight:'1px solid #1f2937', display:'flex', flexDirection:'column', transition:'width 0.2s cubic-bezier(.4,0,.2,1)', overflow:'hidden', zIndex:100, flexShrink:0, fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{ height:56, padding:collapsed?'0':'0 12px 0 16px', display:'flex', alignItems:'center', justifyContent:collapsed?'center':'space-between', borderBottom:'1px solid #1f2937', flexShrink:0 }}>
        ...
        {collapsed && (
          <div style={{ width:30, height:30, background:'var(--green)', borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }} onClick={() => setCollapsed(false)}>
            <Radar size={15} color="#1a0000" strokeWidth={2.5}/>
          </div>
        )}
        {!collapsed && (
      ...
      <nav style={{ flex:1, overflowY:'auto', padding:'8px 0', scrollbarWidth:'none' }}>
              ...
              return (
                <div key={item.id} style={{ padding: collapsed ? '2px 8px' : '2px 8px' }}>
                  <button onClick={() => setPage(item.id)} title={collapsed ? item.label : undefined}
                    ...
                    {active && !collapsed && <span style={{ position:'absolute', left:0, top:'20%', bottom:'20%', width:3, borderRadius:'0 2px 2px 0', background:'var(--green)' }}/>}
                    <item.icon size={15} style={{ flexShrink:0, color:active?'var(--green)':'var(--t3)' }}/>
        ...
        {!user && (
          <div style={{ padding:'16px 10px', display:'flex', flexDirection:'column', gap:7 }}>
            <button onClick={() => setPage('auth')} style={{ padding:'9px', background:'var(--green)', color:'#fff', border:'none', borderRadius:8, fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:"'Inter',system-ui" }}>Start free</button>
        ...
        <div style={{ borderTop:'1px solid #1f2937', padding:collapsed?'6px 0':'6px 8px', flexShrink:0 }}>
          <div style={{ padding:'2px 0', marginBottom:4 }}>
            <button onClick={() => setPage('settings')} title={collapsed?'Settings':undefined}
              ...
              <Settings size={15} style={{ flexShrink:0, color:page==='settings'?'var(--green)':'var(--t3)' }}/>
              {!collapsed && 'Settings'}
            </button>
          </div>
          ...
          <div style={{ position:'relative' }}>
            <button onClick={() => setUserOpen(o=>!o)}
              ...
              <div style={{ width:26, height:26, borderRadius:'50%', background:'rgba(232,137,122,0.15)', color:'var(--green)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, fontWeight:700, flexShrink:0, border:'1px solid var(--green-bdr)' }}>
