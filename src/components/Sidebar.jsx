...
export default function Sidebar({ page, setPage, alertCount = 0, user }) {
  ...
  return (
    <aside style={{ width:w, minWidth:w, height:'100vh', position:'sticky', top:0, background:SB, display:'flex', flexDirection:'column', transition:'width 0.2s ease', overflow:'hidden', zIndex:100, flexShrink:0, fontFamily:"'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      {/* Logo */}
      <div style={{ height:56, padding:collapsed?'0 12px':'0 14px', display:'flex', alignItems:'center', justifyContent:collapsed?'center':'space-between', borderBottom:`1px solid ${SD}`, flexShrink:0 }}>
        {!collapsed && (
              ...
              return (
                <div key={item.id} style={{ padding:'1px 8px' }}>
                  <button onClick={() => setPage(item.id)} title={collapsed ? item.label : undefined}
                    ...
                    {hasBadge && !collapsed && <span style={{ fontSize:10, padding:'1px 6px', borderRadius:10, background:'rgba(255,255,255,0.2)', color:'#fff', fontWeight:700 }}>{alertCount > 99 ? '99+' : alertCount}</span>}
                    {hasBadge && collapsed && <span style={{ position:'absolute', top:6, right:6, width:6, height:6, borderRadius:'50%', background:'#fff' }}/>}
                  </button>
                </div>
              )
            })}
        ...
        <div style={{ borderTop:`1px solid ${SD}`, padding:collapsed?'6px 0':'6px 8px', flexShrink:0 }}>
          ...
          <div style={{ position:'relative' }}>
            <button onClick={() => setUserOpen(o=>!o)}
              style={{ width:'100%', display:'flex', alignItems:'center', gap:9, padding:collapsed?'7px 0':'8px 10px', justifyContent:collapsed?'center':'flex-start', background:'rgba(255,255,255,0.15)', border:'none', borderRadius:7, cursor:'pointer', transition:'background 0.12s' }}
